...
The first deploy of this project failed because `src/` never reached the repo —
GitHub's drag-and-drop web uploader does not reliably carry nested folders.
`MANIFEST.txt` lists every file with a checksum so you can prove a checkout is
whole before you push:

```bash
shasum -a 256 -c MANIFEST.txt --quiet && echo "ALL FILES OK"
git ls-files | wc -l          # expect 48 once MANIFEST.txt is committed too
```

`npm run build` also refuses to start if anything critical is missing, and names
